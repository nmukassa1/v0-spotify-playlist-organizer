...
export function DashboardView({
  ...
  const yearRangeFromSongs =
    songs.length > 0
      ...
      : "—";
  const yearRange = releasedYearSummary
    ? releasedYearSummary.range
    : yearRangeFromSongs;
  const avgPopularity =
    songs.length > 0
      ? Math.round(songs.reduce((a, s) => a + s.popularity, 0) / songs.length)
      : 0;
  const avgDuration =
    songs.length > 0
      ? (() => {
          const avg =
            songs.reduce((a, s) => a + s.durationSeconds, 0) / songs.length;
